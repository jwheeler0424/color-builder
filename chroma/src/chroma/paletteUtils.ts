import type { HarmonyMode, ColorStop, PaletteSlot, SavedPalette } from './types'
import { hexToRgb, rgbToHex, rgbToHsl, hslToRgb, clamp, colorDist } from './colorMath'
...
function anchorHues(mode: HarmonyMode, h: number): number[] {
  switch (mode) {
    ...
    case 'tetradic':
    case 'square':        return [h, rotateHue(h, 90), rotateHue(h, 180), rotateHue(h, 270)]
...
export function genPalette(
  ...
  const base = seeds?.length
    ? seeds[0]
    : { h: Math.random() * 360, s: 40 + Math.random() * 50, l: 35 + Math.random() * 35 }

  if (mode === 'monochromatic') {
    return Array.from({ length: count }, (_, i) => {
      const t = count === 1 ? 0.5 : i / (count - 1)
      return makeStop({ h: base.h, s: clamp(base.s + (Math.random() - 0.5) * 12, 10, 95), l: 15 + t * 70 })
    })
  }
  if (mode === 'shades') {
    return Array.from({ length: count }, (_, i) => {
      const t = count === 1 ? 0.5 : i / (count - 1)
      return makeStop({ h: base.h, s: clamp(base.s - t * 20, 10, 95), l: 8 + t * 82 })
    })
  }
  if (mode === 'natural') {
    return Array.from({ length: count }, () =>
      makeStop({ h: rotateHue(base.h + (Math.random() - 0.5) * 50, 0), s: 15 + Math.random() * 45, l: 25 + Math.random() * 50 })
    )
  }
  if (mode === 'random') {
    const bh = Math.random() * 360
    return Array.from({ length: count }, (_, i) =>
      makeStop({ h: rotateHue(bh + i * 137.508, 0), s: 45 + Math.random() * 45, l: 35 + Math.random() * 35 })
    )
  }
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const lv = total > 1 ? (cycle / (total - 1)) * 40 - 20 : (Math.random() - 0.5) * 22
    return makeStop({
      h: hues[i % hues.length],
...
export function cloneSlot(slot: PaletteSlot): PaletteSlot {
  return {
    locked: slot.locked,
    color: { hex: slot.color.hex, rgb: { ...slot.color.rgb }, hsl: { ...slot.color.hsl } },
  }
}
...
function medianCut(px: Pixel[], depth: number): Pixel[][] {
  ...
  const rR = mxR - mnR, gR = mxG - mnG, bR = mxB - mnB
...
function dedupColors(cs: Pixel[], threshold = 0.08): Pixel[] {
  const r: Pixel[] = []
  for (const c of cs) if (!r.some(x => colorDist(c, x) < threshold)) r.push(c)
  return r
}
...
export function savePalette(name: string, hexes: string[], mode: HarmonyMode): SavedPalette {
  const saved = loadSaved()
  const entry: SavedPalette = { id: crypto.randomUUID(), name, hexes, mode, createdAt: Date.now() }
...
export function loadPrefs(): { mode: HarmonyMode; count: number } | null {
  try { return JSON.parse(localStorage.getItem(LS_PREF_KEY) || 'null') }
  catch { return null }
}
