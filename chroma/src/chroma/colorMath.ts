import type { RGB, HSL, HSV, CMYK, OKLab, OKLCH, UtilityRole, UtilityColor, UtilityColorSet, ThemeTokenSet } from './types'
...
export function mixOklab(a: RGB, b: RGB, t: number): RGB {
  ...
  const s_ = mixed.L - 0.0894841775 * mixed.a - 1.2914855480 * mixed.b
  ...
  const bl2 = clamp(-0.0041960863 * l3 - 0.7034186147 * m3 + 1.7076147010 * s3, 0, 1)
  return {
...
export function scorePalette(slots: { color: { rgb: RGB; hsl: HSL } }[]): PaletteScore {
  if (slots.length < 2) return { balance: 0, accessibility: 0, harmony: 0, uniqueness: 0, overall: 0 }
  ...
  let passPairs = 0, totalPairs = 0
  for (let i = 0; i < slots.length; i++)
    for (let j = i + 1; j < slots.length; j++) {
      totalPairs++
      if (contrastRatio(slots[i].color.rgb, slots[j].color.rgb) >= 4.5) passPairs++
    }
  ...
  const satDev = Math.sqrt(sats.reduce((acc, s) => acc + (s - avgS) ** 2, 0) / sats.length)
...
const UTILITY_DEFS: Record<UtilityRole, { label: string; description: string; anchorHue: number }> = {
  info:    { label: 'Info',    description: 'Informational messages, tooltips, hints',           anchorHue: 210 },
  success: { label: 'Success', description: 'Confirmations, completed states, positive actions',  anchorHue: 145 },
  warning: { label: 'Warning', description: 'Cautions, pending states, non-critical alerts',      anchorHue: 42  },
  error:   { label: 'Error',   description: 'Destructive actions, validation failures, danger',   anchorHue: 5   },
  neutral: { label: 'Neutral', description: 'Disabled states, placeholders, secondary content',   anchorHue: 0   },
  focus:   { label: 'Focus',   description: 'Keyboard focus rings, interactive highlights',       anchorHue: 0   },
}

export function generateUtilityColors(
  slots: { color: { hex: string; rgb: RGB; hsl: HSL } }[]
): UtilityColorSet {
  const avgSat = slots.length ? slots.reduce((s, sl) => s + sl.color.hsl.s, 0) / slots.length : 60
  const avgLit = slots.length ? slots.reduce((s, sl) => s + sl.color.hsl.l, 0) / slots.length : 50
  ...
  function nudgeAwayFromPalette(hue: number): number {
    ...
    const nearest = slots.reduce((best, sl) => {
      const d = Math.min(Math.abs(sl.color.hsl.h - hue), 360 - Math.abs(sl.color.hsl.h - hue))
  ...
  function makeStop(hue: number, sat: number, lit: number): { hex: string; rgb: RGB; hsl: HSL } {
  ...
  function makeUtility(role: UtilityRole, hue: number): UtilityColor {
    ...
    const sat = role === 'neutral' ? clamp(targetSat * 0.18, 6, 20)
              : role === 'warning' ? clamp(targetSat * 0.9, 55, 88)
              : targetSat
    const lit = role === 'warning' ? clamp(targetLit + 6, 44, 62)
              : role === 'neutral' ? clamp(targetLit + 4, 44, 62)
              : targetLit
    return { ...def, role, color: makeStop(hue, sat, lit), locked: false }
  }

  return {
    info:    makeUtility('info',    nudgeAwayFromPalette(UTILITY_DEFS.info.anchorHue)),
    success: makeUtility('success', nudgeAwayFromPalette(UTILITY_DEFS.success.anchorHue)),
    warning: makeUtility('warning', nudgeAwayFromPalette(UTILITY_DEFS.warning.anchorHue)),
    error:   makeUtility('error',   nudgeAwayFromPalette(UTILITY_DEFS.error.anchorHue)),
    neutral: makeUtility('neutral', dominantHue),
    focus:   makeUtility('focus',   nudgeAwayFromPalette((dominantHue + 60) % 360)),
  }
}

export function mergeUtilityColors(existing: UtilityColorSet, generated: UtilityColorSet): UtilityColorSet {
...
export function deriveThemeTokens(
  ...
  if (!slots.length) return { semantic: [], utility: {} as ThemeTokenSet['utility'], palette: [] }

  const byLit = [...slots].sort((a, b) => a.color.hsl.l - b.color.hsl.l)
  const bySat = [...slots].sort((a, b) => b.color.hsl.s - a.color.hsl.s)
  const accentHsl = bySat[0].color.hsl
  const accentDark  = rgbToHex(hslToRgb({ h: accentHsl.h, s: clamp(accentHsl.s, 55, 95), l: clamp(accentHsl.l, 45, 65) }))
  const accentLight = rgbToHex(hslToRgb({ h: accentHsl.h, s: clamp(accentHsl.s, 55, 85), l: clamp(accentHsl.l - 8, 35, 55) }))
  const baseHue = byLit[0].color.hsl.h
  const mkN = (l: number, s = 4) => rgbToHex(hslToRgb({ h: baseHue, s, l }))

  const semantic = [
    { name: 'color-bg',             light: mkN(98),       dark: mkN(7),         description: 'Page / canvas background' },
    { name: 'color-bg-subtle',      light: mkN(94),       dark: mkN(11),        description: 'Subtle background (stripes, hover)' },
    { name: 'color-surface',        light: mkN(100),      dark: mkN(14),        description: 'Card / panel background' },
    { name: 'color-surface-raised', light: mkN(100),      dark: mkN(18),        description: 'Raised surface (popover, tooltip)' },
    { name: 'color-border',         light: mkN(88),       dark: mkN(22),        description: 'Default border' },
    { name: 'color-border-strong',  light: mkN(78),       dark: mkN(32),        description: 'Strong / focus border' },
    { name: 'color-text',           light: mkN(9),        dark: mkN(96),        description: 'Primary text' },
    { name: 'color-text-muted',     light: mkN(42),       dark: mkN(58),        description: 'Secondary / muted text' },
    { name: 'color-text-disabled',  light: mkN(62),       dark: mkN(38),        description: 'Disabled text' },
    { name: 'color-accent',         light: accentLight,   dark: accentDark,     description: 'Brand accent / CTA' },
    { name: 'color-accent-subtle',  light: `${accentLight}22`, dark: `${accentDark}22`, description: 'Accent tint background' },
  ]
  ...
  for (const role of roles) {
    ...
    const light  = rgbToHex(hslToRgb({ h, s: clamp(s - 10, 30, 85), l: role === 'warning' ? 38 : 40 }))
    const dark   = rgbToHex(hslToRgb({ h, s: clamp(s, 40, 90),      l: role === 'warning' ? 52 : 54 }))
  ...
  return {
    ...
    palette: slots.map((sl, i) => ({ name: `palette-${i + 1}`, hex: sl.color.hex })),
  }
}

export function buildThemeCss(tokens: ThemeTokenSet): string {
  const semLight = tokens.semantic.map(t => `  --${t.name}: ${t.light};`).join('\n')
  const semDark  = tokens.semantic.map(t => `  --${t.name}: ${t.dark};`).join('\n')
  const utilVars = (mode: 'light' | 'dark') => Object.entries(tokens.utility)
    .map(([role, v]) => `  --${role}: ${mode === 'light' ? v.light : v.dark};\n  --${role}-subtle: ${v.subtle};`)
    .join('\n')
  const palVars  = tokens.palette.map(p => `  --${p.name}: ${p.hex};`).join('\n')
...
export function buildFigmaTokens(tokens: ThemeTokenSet, utility: UtilityColorSet): string {
  const obj = {
    global: Object.fromEntries(tokens.palette.map(p => [p.name, { value: p.hex, type: 'color' }])),
    semantic: {
      light: Object.fromEntries(
        tokens.semantic.map(t => [t.name.replace('color-', ''), { value: t.light, type: 'color', description: t.description }])
      ),
      dark: Object.fromEntries(
        tokens.semantic.map(t => [t.name.replace('color-', ''), { value: t.dark, type: 'color', description: t.description }])
      ),
    },
    utility: Object.fromEntries(
      Object.entries(tokens.utility).map(([role, v]) => [
        role,
        {
          DEFAULT: { value: v.base,   type: 'color', description: utility[role as UtilityRole].description },
...
export function buildTailwindConfig(tokens: ThemeTokenSet, _utility: UtilityColorSet): string {
  ...
  const palSection = tokens.palette.map(p => `      '${p.name}': '${p.hex}',`).join('\n')
  const semSection = tokens.semantic.map(t => `      '${t.name.replace('color-', '')}': 'var(--${t.name})',`).join('\n')
