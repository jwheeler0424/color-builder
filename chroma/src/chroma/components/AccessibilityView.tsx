...
import { contrastRatio, wcagLevel, type WcagLevel } from '../colorMath'
...
export default function AccessibilityView() {
  ...
  if (!slots.length) {
    return (
      <div className="ch-view-scroll ch-view-pad">
        <div className="ch-view-hd"><h2>Accessibility Matrix</h2></div>
  ...
  // Build passing pairs
  const pairs: { a: typeof slots[0]; b: typeof slots[0]; ratio: number; level: WcagLevel }[] = []
  for (let i = 0; i < slots.length; i++) {
    for (let j = i + 1; j < slots.length; j++) {
      const ratio = contrastRatio(slots[i].color.rgb, slots[j].color.rgb)
      const level = wcagLevel(ratio)
      if (level !== 'Fail') pairs.push({ a: slots[i], b: slots[j], ratio, level })
    }
  }
    ...
    <div className="ch-view-scroll ch-view-pad">
      <div style={{ maxWidth: 920, margin: '0 auto' }}>
                ...
                <tr key={i}>
                  <td>
                    <span style={{
                      ...
                      background: sa.color.hex, borderRadius: 2,
                      border: '1px solid rgba(255,255,255,.08)', verticalAlign: 'middle', marginRight: 4,
                    }} />
                    <span style={{ fontSize: 10, color: 'var(--ch-t3)' }}>{i + 1}</span>
                  </td>
                  {slots.map((sb, j) => {
                    if (i === j) return <td key={j} className="ch-a11y-self">—</td>
                    const ratio = contrastRatio(sa.color.rgb, sb.color.rgb)
                    const level = wcagLevel(ratio)
                    return (
                      <td key={j}>
                        <Badge level={level} />
                        <div style={{ fontSize: 10, color: 'var(--ch-t3)', marginTop: 2 }}>
                          {ratio.toFixed(1)}:1
                        </div>
                      </td>
                    )
        ...
        <div style={{ marginTop: 24 }}>
          <div className="ch-slabel" style={{ marginBottom: 12 }}>Top Passing Pairs</div>
          <div className="ch-a11y-pairs-grid">
            {pairs.slice(0, 8).map(({ a, b, ratio, level }, i) => (
              <div key={i} className="ch-a11y-pair-card">
                <div style={{
                  height: 64, background: a.color.hex,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  flexDirection: 'column', gap: 3, padding: 8,
                }}>
                  <div style={{ fontFamily: 'var(--ch-fd)', fontSize: 14, fontWeight: 700, color: b.color.hex }}>Aa Sample</div>
                  <div style={{ fontSize: 10, color: b.color.hex, opacity: .8 }}>Body text preview</div>
                </div>
                <div style={{ padding: '8px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', gap: 4 }}>
                    {[a, b].map((x, xi) => (
                      <div key={xi} style={{
                        width: 10, height: 10, background: x.color.hex, borderRadius: 2,
                        border: '1px solid rgba(255,255,255,.08)',
                      }} />
                    ))}
                  </div>
                  <span style={{ fontSize: 10, color: 'var(--ch-t3)' }}>{ratio.toFixed(1)}:1</span>
                  <Badge level={level} />
                </div>
              </div>
            ))}
