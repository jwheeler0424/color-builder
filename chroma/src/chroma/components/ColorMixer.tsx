...
import {
  parseHex, hexToRgb, rgbToHex, rgbToHsl, hslToRgb,
  mixOklab, mixHsl, mixRgb, textColor,
} from '../colorMath'
...
const MIX_SPACES: { id: MixSpace; label: string; desc: string }[] = [
  { id: 'oklch', label: 'OKLab',  desc: 'Perceptually uniform — no muddy midpoints' },
...
export default function ColorMixer() {
  ...
  const rgbA = useMemo(() => parseHex(colorA) ? hexToRgb(parseHex(colorA)!) : { r: 230, g: 57, b: 70 }, [colorA])
  const rgbB = useMemo(() => parseHex(colorB) ? hexToRgb(parseHex(colorB)!) : { r: 69, g: 123, b: 157 }, [colorB])
  ...
  const useMixAsSeeds = () => {
    const seeds = blendRow.map(({ rgb, hex }) => ({ hex, rgb, hsl: rgbToHsl(rgb) }))
    setSeeds(seeds)
    generate()
    navigate({ to: '/palette' })
  }
    ...
    <div className="ch-view-scroll ch-view-pad">
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        <div className="ch-view-hd">
          <h2>Color Mixer</h2>
          <p>Blend two colors across different color spaces. OKLab avoids the muddy middle-ground problem.</p>
        </div>
          ...
          {[
            ...
            <div key={label} className="ch-mixer-input-group">
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <div style={{
                  width: 48, height: 48, borderRadius: 4, background: rgbToHex(rgb),
                  border: '2px solid var(--ch-b2)', flexShrink: 0,
                }} />
                <div>
                  <input
                    className="ch-inp"
                    ...
                    spellCheck={false}
                    autoComplete="off"
                    style={{ fontFamily: 'var(--ch-fm)', letterSpacing: '.06em', marginBottom: 4 }}
                  />
                  <div style={{ fontSize: 10, color: 'var(--ch-t3)' }}>{nearestName(rgb)}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        {slots.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div className="ch-slabel" style={{ marginBottom: 6 }}>Pick from Palette (A / B)</div>
              ...
              {slots.map((slot, i) => (
                <div key={i} style={{ position: 'relative' }}>
                  <div
                    ...
                    onContextMenu={e => { e.preventDefault(); setColorB(slot.color.hex) }}
                  />
                </div>
              ))}
              <div style={{ fontSize: 10, color: 'var(--ch-t3)', alignSelf: 'center', marginLeft: 4 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div className="ch-slabel" style={{ marginBottom: 8 }}>Blend Space</div>
        ...
        {/* Active blend strip */}
        <div className="ch-slabel" style={{ marginBottom: 8 }}>Result</div>
        <div className="ch-mixer-strip">
          {blendRow.map(({ hex, rgb }, i) => {
            ...
            return (
              ...
              >
                <span style={{ color: tc, fontSize: 9 }}>{hex.toUpperCase()}</span>
              </div>
            )
          })}
        </div>
        ...
        <div className="ch-slabel" style={{ margin: '24px 0 10px' }}>Space Comparison</div>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
          <Button variant="primary" onClick={useMixAsSeeds}>Use Blend as Seeds →</Button>
          <Button variant="ghost" onClick={addMidpoint}>+ Add Midpoint to Palette</Button>
        </div>
      </div>
    </div>
  )
