...
import {
  parseAny, rgbToHex, rgbToHsl, rgbToHsv, rgbToCmyk,
  rgbToOklab, oklabToLch,
} from '../colorMath'
import { nearestName } from '../paletteUtils'

function ConvCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  ...
  return (
    <div className="ch-conv-card">
      ...
      <button className="ch-conv-copy" onClick={copy}>{copied ? '✓' : 'copy'}</button>
    </div>
  )
}
    ...
    <div className="ch-view-scroll" style={{ padding: 28 }}>
      <div style={{ maxWidth: 660, margin: '0 auto' }}>
        <div className="ch-view-hd">
          ...
          <p>Paste any hex, rgb(), or hsl() value to see all formats instantly.</p>
        </div>
        ...
        <div className="ch-conv-grid">
          ...
          <ConvCard label="CSS RGB" value={`rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`} sub={`R:${rgb.r} G:${rgb.g} B:${rgb.b}`} />
          <ConvCard
            label="CSS HSL"
          ...
          />
          <ConvCard label="OKLab" value={`oklab(${oklab.L.toFixed(3)}, ${oklab.a.toFixed(3)}, ${oklab.b.toFixed(3)})`} />
          <ConvCard
            label="OKLCH"
