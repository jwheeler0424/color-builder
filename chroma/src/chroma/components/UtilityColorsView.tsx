...
import { parseHex, textColor, contrastRatio, wcagLevel, hslToRgb, rgbToHex, rgbToHsl } from '../colorMath'
...
function UtilityCard({
  ...
  useEffect(() => { setEditVal(utility.color.hex) }, [utility.color.hex])
  ...
  const BADGE_COLOR: Record<string, string> = {
    AAA: '#00e676', AA: '#69f0ae', 'AA Large': '#fff176', Fail: '#ff4455',
  }
  const BADGE_BG: Record<string, string> = {
    AAA: 'rgba(0,230,118,.18)', AA: 'rgba(105,240,174,.15)',
    'AA Large': 'rgba(255,241,118,.12)', Fail: 'rgba(255,68,85,.15)',
  }
    ...
    <div className="ch-utility-card">
      ...
      <div className="ch-utility-swatch-row">
        <div className="ch-utility-swatch" style={{ background: hex }}>
          <span style={{ color: tc, fontSize: 18 }}>{ROLE_ICONS[utility.role]}</span>
        </div>
        <div className="ch-utility-meta">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            ...
            <button
              ...
              title={utility.locked ? 'Unlock — will update on next generate' : 'Lock — protect from re-generation'}
            >
              {utility.locked ? '🔒' : '🔓'}
            </button>
          </div>
      ...
      <input
        ...
        spellCheck={false}
        autoComplete="off"
        style={{ fontFamily: 'var(--ch-fm)', letterSpacing: '.06em', fontSize: 11, marginTop: 10 }}
      />
      ...
      <div className="ch-utility-previews">
        ...
        <div className="ch-utility-preview-item">
          <span style={{ fontSize: 10, color: 'var(--ch-t3)', marginBottom: 4, display: 'block' }}>Filled</span>
          <span style={{
            background: hex, color: tc, padding: '3px 10px',
            borderRadius: 12, fontSize: 11, fontWeight: 700, display: 'inline-flex',
        ...
        {/* Subtle badge */}
        <div className="ch-utility-preview-item">
          <span style={{ fontSize: 10, color: 'var(--ch-t3)', marginBottom: 4, display: 'block' }}>Subtle</span>
          <span style={{
            background: subtleBg, color: subtleText, padding: '3px 10px',
            borderRadius: 12, fontSize: 11, fontWeight: 700, display: 'inline-flex',
            alignItems: 'center', gap: 5,
          }}>
            {ROLE_ICONS[utility.role]} {utility.label}
        ...
        {/* Outline badge */}
        <div className="ch-utility-preview-item">
          <span style={{ fontSize: 10, color: 'var(--ch-t3)', marginBottom: 4, display: 'block' }}>Outline</span>
          <span style={{
            border: `1.5px solid ${hex}`, color: hex, padding: '2px 9px',
            borderRadius: 12, fontSize: 11, fontWeight: 700, display: 'inline-flex',
            alignItems: 'center', gap: 5, background: 'transparent',
          }}>
            {ROLE_ICONS[utility.role]} {utility.label}
          </span>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8 }}>
        <span style={{
          ...
          background: BADGE_BG[level], color: BADGE_COLOR[level],
        }}>
          {level}
        </span>
...
function LivePreview() {
  ...
  const bgHex = slots.length > 0
    ? [...slots].sort((a, b) => a.color.hsl.l - b.color.hsl.l)[0].color.hex
    : '#0f0f0f'
  const surfaceHex = slots.length > 1
  ...
  return (
    <div className="ch-utility-live-preview" style={{ background: bgHex }}>
      <div style={{ fontSize: 10, color: 'var(--ch-t3)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 12 }}>
        Live Component Preview
      </div>
      {/* Alert banners */}
      {roles.map(role => {
        const u = utilityColors[role]
        const subtle = rgbToHex(hslToRgb({ h: u.color.hsl.h, s: Math.max(u.color.hsl.s * 0.2, 6), l: 14 }))
        const subtleBorder = rgbToHex(hslToRgb({ h: u.color.hsl.h, s: Math.min(u.color.hsl.s * 0.6, 60), l: 28 }))
        const subtleText = rgbToHex(hslToRgb({ h: u.color.hsl.h, s: Math.min(u.color.hsl.s, 75), l: 78 }))
        return (
          <div key={role} style={{
            background: subtle, border: `1px solid ${subtleBorder}`,
            borderRadius: 6, padding: '10px 14px', marginBottom: 8,
            display: 'flex', alignItems: 'flex-start', gap: 10,
          }}>
            <span style={{ color: u.color.hex, fontSize: 14, flexShrink: 0, marginTop: 1 }}>
              {ROLE_ICONS[role]}
            </span>
            <div>
              <div style={{ color: subtleText, fontWeight: 700, fontSize: 12, marginBottom: 2 }}>
                {u.label} alert
              </div>
        ...
        {(['neutral', 'focus'] as UtilityRole[]).map(role => {
          ...
          return (
            <button
              key={role}
              style={{
                background: surfaceHex, border: `2px solid ${role === 'focus' ? u.color.hex : 'transparent'}`,
                outline: role === 'focus' ? `2px solid ${u.color.hex}` : 'none',
                outlineOffset: 2,
                borderRadius: 4, padding: '6px 14px', cursor: 'pointer',
                fontSize: 12, color: textColor(hexToStop(surfaceHex).rgb),
...
export default function UtilityColorsView() {
  const { utilityColors, slots, setUtilityColor, toggleUtilityLock, regenUtilityColors } = useChromaStore()
    ...
    <div className="ch-view-scroll ch-view-pad">
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        <div className="ch-view-hd">
          ...
          <p>
            Semantic UI colors derived from your palette. They share the palette's character
            while remaining perceptually distinct. Lock any color to protect it during re-generation.
          </p>
        </div>

        <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap' }}>
          <Button variant="primary" onClick={regenAll}>↻ Regenerate All</Button>
          <Button variant="ghost" onClick={copyCss}>{cssCopied ? '✓ Copied CSS' : 'Copy CSS Vars'}</Button>
        </div>
        ...
        <div style={{ marginTop: 32 }}>
          <div className="ch-slabel" style={{ marginBottom: 12 }}>Component Preview</div>
          <LivePreview />
        </div>

        {/* Quick CSS vars */}
        <div style={{ marginTop: 24 }}>
          <div className="ch-slabel" style={{ marginBottom: 8 }}>CSS Variables</div>
