...
import { hslToRgb, clamp } from '../../colorMath'
...
export default function ColorWheel({ hsl, size = 260, onChange }: ColorWheelProps) {
  ...
  const drawWheel = useCallback(() => {
    ...
    const cx = sz / 2, cy = sz / 2, radius = sz / 2 - 2
  ...
  // Initial draw
  useEffect(() => { drawWheel() }, []) // eslint-disable-line react-hooks/exhaustive-deps

  const hitWheel = useCallback((clientX: number, clientY: number) => {
    const canvas = canvasRef.current
    if (!canvas) return
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => { if (draggingRef.current) hitWheel(e.clientX, e.clientY) }
    const onTMove = (e: TouchEvent) => {
      if (draggingRef.current) { e.preventDefault(); hitWheel(e.touches[0].clientX, e.touches[0].clientY) }
    }
    const onUp = () => { draggingRef.current = false; rectRef.current = null }
