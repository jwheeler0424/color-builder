...
import { textColor, parseHex } from '../colorMath'
import { nearestName, hexToStop } from '../paletteUtils'
import { HARMONIES, THEMES } from '../constants'
  ...
  return (
    <div className="ch-slot" style={{ background: slot.color.hex }}>
      ...
      <div className="ch-slot-acts">
        ...
        <button className="ch-sbtn" style={{ color: tc }} onClick={handleCopy} title="Copy hex">📋</button>
      </div>
      <div className="ch-slot-info">
        <div className="ch-slot-name" style={{ color: tc }}>{name}</div>
        ...
        <div className="ch-slot-hsl" style={{ color: tc }}>
          {Math.round(slot.color.hsl.h)}° {Math.round(slot.color.hsl.s)}% {Math.round(slot.color.hsl.l)}%
        </div>
      </div>
    </div>
  )
...
function SlotEditPopover({ slotHex, onClose, onApply }: {
  slotHex: string; onClose: () => void; onApply: (hex: string) => void
}) {
  ...
  const handleChange = (v: string) => {
    setValue(v)
    const h = parseHex(v)
    if (h) { setError(false); onApply(h) } else setError(true)
  }
  return (
    <div className="ch-slot-edit-popover" onClick={e => e.stopPropagation()}>
      <div className="ch-slot-edit-swatch" style={{ background: parseHex(value) || slotHex }} />
      <input
        className={`ch-inp${error ? ' err' : ''}`}
        value={value}
        onChange={e => handleChange(e.target.value)}
        maxLength={7} spellCheck={false} autoFocus
        style={{ fontFamily: 'var(--ch-fm)', letterSpacing: '.06em', width: 100 }}
      />
...
export default function PaletteView() {
  const {
    slots, seeds, count, mode,
    generate, setMode, setCount, addSeed, removeSeed, setSeeds, editSlotColor,
  } = useChromaStore()
  ...
  const handleAddSeed = useCallback(() => {
    ...
    if (!hex) { setSeedErr(true); setTimeout(() => setSeedErr(false), 600); return }
  ...
  return (
    <div className="ch-view-pal">
      <div className="ch-pstrip" onClick={() => editingSlot !== null && setEditingSlot(null)}>
        {slots.map((slot, i) => (
          <div key={i} style={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column' }}>
            ...
            <button
              className="ch-sbtn"
              style={{ position: 'absolute', bottom: 8, right: 8, color: textColor(slot.color.rgb) }}
      ...
      <aside className="ch-panel">
        <div className="ch-pscroll">
          <div className="ch-psec">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <input type="range" min={4} max={12} value={count}
                onChange={e => setCount(+e.target.value)} className="ch-range" />
          ...
          <div className="ch-psec">
            ...
            <div className="ch-themes-grid">
              {THEMES.map((t, i) => (
                <div key={i} className="ch-theme-card" title={t.name}
                  onClick={() => {
                    setMode(t.mode)
                ...
                >
                  <div className="ch-theme-strip">
                    {t.seeds.slice(0, 5).map((h, j) => (
                      <div key={j} className="ch-theme-chip" style={{ background: h }} />
                    ))}
                  </div>
          ...
          <div className="ch-psec">
            ...
            <div className="ch-seedlist">
              {seeds.map((s, i) => (
                <div key={i} className="ch-seeditem">
                  ...
                  <button className="ch-seedrm" onClick={() => removeSeed(i)}>×</button>
                </div>
              ))}
            </div>
            <div className="ch-seedrow">
              <input
                ...
                placeholder="#F4A261" maxLength={7} spellCheck={false} autoComplete="off"
                style={{ fontSize: 11 }}
              />
              <Button variant="ghost" size="sm" onClick={handleAddSeed}>+ Add</Button>
            </div>
          </div>

          <div className="ch-psec">
            <div className="ch-slabel">Harmony</div>
            <div className="ch-hdesc">{HARMONIES.find(h => h.id === mode)?.desc || ''}</div>
            <div className="ch-hgrid">
              {HARMONIES.map(h => (
                <button
                  key={h.id}
                  className={`ch-hbtn${mode === h.id ? ' on' : ''}`}
                  onClick={() => { setMode(h.id); generate() }}
                >
                  {h.label}
                </button>
              ))}
          ...
          {slots.length > 0 && (
            <div className="ch-psec">
              ...
              <div className="ch-prevstrip">
                {slots.map((s, i) => <div key={i} style={{ flex: 1, background: s.color.hex }} />)}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="ch-genbar">
          <Button variant="primary" className="ch-gen-btn" onClick={generate}>⟳ Generate</Button>
          <div className="ch-hint">Press <kbd>Space</kbd> to generate · <kbd>Ctrl+Z</kbd> to undo</div>
        </div>
      </aside>
    </div>
  )
