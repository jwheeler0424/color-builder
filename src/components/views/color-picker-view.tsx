...
export default function ColorPickerView() {
  ...
  const alphaHex = Math.round((pickerAlpha / 100) * 255)
    .toString(16)
    .padStart(2, "0");
        ...
        <div className="ch-sliders">
          <div className="ch-slider-row">
            <div className="ch-slider-label">
              Hue <span>{Math.round(pickerHsl.h)}°</span>
            </div>
          ...
          <div className="ch-slider-row">
            <div className="ch-slider-label">
              Saturation <span>{Math.round(pickerHsl.s)}%</span>
            </div>
            <div
              className="ch-sat-track"
              style={{
          ...
          <div className="ch-slider-row">
            <div className="ch-slider-label">
              Lightness <span>{Math.round(pickerHsl.l)}%</span>
            </div>
            <div
              className="ch-lit-track"
              style={{
      ...
      <div className="ch-picker-panel">
        ...
        <div>
          ...
          <div className="ch-picker-info">
            <div>
              Name <strong>{nearestName(rgb)}</strong>
            </div>
            <div>
              HEX <strong>{displayHex.toUpperCase()}</strong>
            </div>
            <div>
              RGB{" "}
              <strong>
            ...
            <div>
              Luminance <strong>{(luminance(rgb) * 100).toFixed(1)}%</strong>
            </div>
            <div>
              OKLCH{" "}
              <strong>
                {lch.L.toFixed(1)}% {(lch.C / 100).toFixed(3)}{" "}
                {Math.round(lch.H)}°
              </strong>
            </div>
          </div>
